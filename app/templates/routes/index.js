(function (exports) {
  ...
  exports.init = function (app, passport, sessionStore) {
    ...
    var checkForSessionTimeout = function (req, res, next) {
      ...
      sessionStore.load(sessionID, function(err, session) {
        if ((err || !session) && req.xhr) {
          res.status(500).json({error: '<%= namespace %> session timeout'});
    ...
    app.get('/login',
      function (req, res) {
        res.render('login', {'title': 'New to <%= namespace %>?',
    ...
    app.get('/board/:boardId/:slug?', checkForSessionTimeout,
      ensureAuthenticated, function (req, res) {
        boardHandler.boardExists(req.params.boardId, function(err, board) {
          if (board) {
            res.render('application', {title: '<%= namespace %>'});
          } else {
            res.status(404).render('404', {title: '<%= namespace %> | 404', url: req.url});
          }
        });
      });
    ...
    app.get('/standalonehelp', function (req, res) {
      res.render('standalone-help', {title: '<%= namespace %>'});
    });
