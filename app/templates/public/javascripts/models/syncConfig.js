...
(function ($, _, Backbone) {
  ...
  <%= namespace %>.models.SyncConfig = <%= namespace %>.models.BaseModel.extend({
    idAttribute: "_id",
    noIoBind: false,
  ...
  //Collection
  <%= namespace %>.models.SyncConfigCollection = <%= namespace %>.models.BaseCollection.extend({
    model: <%= namespace %>.models.SyncConfig,
    socket: <%= namespace %>.socket,
    ...
    serverCreate: function (data) {
      if (data) {
        var board = <%= namespace %>.utils.getCurrentBoardModel();
        if (board) {
