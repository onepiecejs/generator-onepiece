...
(function ($, _, Backbone) {
  ...
  <%= namespace %>.views.EntryView = Backbone.View.extend({
    tagName: "div",
   ...
   *  MoveCardToView
   **/
  window.<%= namespace %>.MoveCardToView = Backbone.View.extend({
    ...
    initialize: function(data) {
      ...
      this.boardId = <%= namespace %>.utils.getCurrentBoardId();
      this.listId = data.listId;
      this.title = data.title;
      this.cardId = data.cardId;
      this.boardCollection = new <%= namespace %>.models.BoardCollection();
      this.listCollection = new <%= namespace %>.models.ListCollection();
      this.cardColleciton = new <%= namespace %>.models.CardCollection();
      this.boardMemberCollection = new <%= namespace %>.models.BoardMemberCollection();
    },
    ...
    moveAction: function(event) {
      ...
      <%= namespace %>.socket.emit("move-card", moveData);
      this.$el.modal('hide');
    },

    moveSearch: function(event) {
      <%= namespace %>.utils.renderMoveSearch(event);
    },

    updateCheckedList: function(event) {
      var baordId = $(event.target).data('itemid');
      var tabIndex = 1;
      <%= namespace %>.utils.renderMoveList(this, baordId, this.listId, tabIndex);
    },
      ...
      this.cardColleciton.fetch({
        ...
        success: function(cards, response, options) {
          ...
          cards.each(function(card) {
            var itemView = new <%= namespace %>.views.MoveItemView({
              model: {
                "_id": card.id,
    ...
    updateMoveList: function(tabs) {
      ...
      tabs.forEach(function(tab) {
        ...
        <%= namespace %>.utils.renderMoveList(_this, _this.boardId, _this.listId, tabIndex);
      });
    },
  ...
  <%= namespace %>.views.CardView = Backbone.View.extend({
    tagName: "div",
    ...
    initialize: function() {
      ...
      this.cardLabelCollection = new <%= namespace %>.models.CardLabelRelationCollection();
      this.voteCollection = new <%= namespace %>.models.VoteCollection();
    },
    ...
    render: function() {
      ...
      this.cardNeonLightsView = new <%= namespace %>.views.LabelNeonLightsView({
      ...
      if (!window.<%= namespace %>.isBoardMember) {
        this.disableEvents();
      }
    ...
    isArchivedChanged: function(data) {
      ...
      var viewIdArray = _.map(
        <%= namespace %>.utils.getCurrentBoardView().listViewCollection,
        function(child) {
      ...
      var inListView = <%= namespace %>.utils.getCurrentBoardView().listViewCollection[inListViewIndex];
    ...
    showCardDetail: function (event) {
      ...
      <%= namespace %>.appRouter.navigate("card/" + this.model.id + "/" + $.slug(this.model.get("title")));

      this.cardDetailView = new <%= namespace %>.views.CardDetailsView({
        model: this.model,
    ...
    showCardMenu: function (event) {
      ...
      if (!this.cardMenuView) {
        this.cardMenuView = new <%= namespace %>.views.CardMenuView();
      }

      $("body").click();

      var offsetX = <%= namespace %>.utils.getOffsetX(event.pageX, "#card-menu");
      ...
      this.attributes.expandedViewChain = <%= namespace %>.utils.rememberMe(
        this,
    ...
    collapse: function() {
      ...
      this.attributes.expandedViewChain = <%= namespace %>.utils.forgetMe(
        this,
        this.attributes.expandedViewChain
      );
    },
    ...
    moveFrom: function(event, ui, fromListView) {
      ...
      <%= namespace %>.views.fromListView = fromListView;
    ...
    moveIn: function(event, ui, inListView) {
      ...
      <%= namespace %>.views.inListView = inListView;
    },
    ...
    refreshCardOrder: function() {
      ...
      var fromListView = <%= namespace %>.utils.getCurrentBoardView().listViewCollection[fromListViewIndex];
  ...
  // extend `CardDetailsView` from `<%= namespace %>.views.CardView`.
  <%= namespace %>.views.CardDetailsView = <%= namespace %>.views.CardView.extend({
    ...
    render: function () {
      ...
      $.when(this.$el.html(this.template({ 'card': card }))).done(function() {
        _this.$el.modal();
        <%= namespace %>.setTitle("Card|" + _this.model.get("title"));
        ...
        _this.cardVotesTotalView = new <%= namespace %>.views.CardVotesTotalView({
        ...
        _this.checklistSectionView = new <%= namespace %>.views.ChecklistSectionView({
          el: _this.$el.find('section.js-checklist-section'),
          collection: new <%= namespace %>.models.ChecklistCollection([], { 'card': _this.model }),
          card: _this.model
        });
        _this.checklistSectionView.render();

        _this.detailsNeonLightsView = new <%= namespace %>.views.LabelNeonLightsView({
        ...
        _this.fileupload = _this.$el.find('#attachmentUpload').fileupload({
          ...
          var newAttachmentUploadItemView = new <%= namespace %>.views.AttachmentUploadItemView({
            'model': new <%= namespace %>.models.Attachment(data.files[0]),
            'data': data,
            'parentView': _this
          });
          ...
          if (data.result.user_error) {
            ...
            var newAttachment = new <%= namespace %>.models.Attachment(data.result.attachment);
            newAttachment.save();
          }
    ...
    onSubscribeClick: function() {
      ...
      var currentUserId = <%= namespace %>.utils.getCurrentUser().id;
    ...
    renderCommentView: function() {
      this.commentView = new <%= namespace %>.views.CommentView({
    ...
    renderAttachmentView: function() {
      this.attachmentView = new <%= namespace %>.views.AttachmentView({
    ...
    renderAssignView: function() {
      this.cardAssignView = new <%= namespace %>.views.CardAssignView({
    ...
    toggleAssignWindow: function(event) {
      ...
      if (this.cardAssignView.isExpanded === false) {
        ...
        this.cardAssignView.attributes.expandedViewChain = <%= namespace %>.utils.rememberMe(
          this.cardAssignView,
    ...
    renderLabelView: function() {
      this.labelAssignView = new <%= namespace %>.views.LabelAssignView({
    ...
    toggleLabelWindow: function(event) {
      ...
      if (this.labelAssignView.isExpanded === false) {
        ...
        $('.modal-scrollable').scrollTop(0);
        this.labelAssignView.attributes.expandedViewChain = <%= namespace %>.utils.rememberMe(
    ...
    renderVoteView: function() {
      this.cardVoteView = new <%= namespace %>.views.CardVoteView({
    ...
    toggleVoteWindow: function(event) {
      ...
      if (this.cardVoteView.isExpanded === false) {
        ...
        $('.modal-scrollable').scrollTop(0);
        this.cardVoteView.attributes.expandedViewChain = <%= namespace %>.utils.rememberMe(
          this.cardVoteView,
    ...
    canComment: function() {
      var commentStatus = <%= namespace %>.utils.getCurrentCommentStatus();
      var canComment = true;
      if (commentStatus === 'disabled') {
        canComment = false;
      } else if (commentStatus === 'enabled' && !window.<%= namespace %>.isBoardMember) {
        canComment = false;
      }
      return canComment;
    },
    ...
    close: function() {
      ...
      var boardId = <%= namespace %>.utils.getCurrentBoardModel().id;
      var boardTitle = <%= namespace %>.utils.getCurrentBoardModel().get("title");
      var slug = $.slug(boardTitle);
      <%= namespace %>.appRouter.navigate("board/" + boardId + "/" + slug);
      <%= namespace %>.setTitle("Board|" + boardTitle);
    ...
    onChecklistClick: function(event) {
      ...
      var view = new <%= namespace %>.views.EntryView({
        ...
        buttons: [{
          ...
          eventHandler: function(event, data) {
            ...
            var checklist = new <%= namespace %>.models.Checklist({
              title: title,
              cardId: data.card.id,
              authorId: <%= namespace %>.utils.getCurrentUser().id
            });
            checklist.save();
  ...
  <%= namespace %>.views.CardAssignView = Backbone.View.extend({
    ...
    _getMembersToAssign: function() {
      ...
      var memberCollection = <%= namespace %>.utils.getCurrentBoardView().memberCollection;
   ...
   */
  <%= namespace %>.views.LabelAssignView = Backbone.View.extend({
  ...
  <%= namespace %>.views.CardMenuView = Backbone.View.extend({
    el: "div#card-menu",
    ...
    _getCardModel: function() {
      return <%= namespace %>.utils.getCardModelById(this.cardId);
    },
    ...
    moveCard: function(event) {
      this.hideCardMenu(event);
      this.moveCardToView = new <%= namespace %>.MoveCardToView({
        title: 'Move Card',
        listId: this.listId,
        cardId: this.cardId
      });
   ...
   */
  <%= namespace %>.views.LabelNeonLightsView = Backbone.View.extend({
  ...
  <%= namespace %>.views.CardVotesTotalView = Backbone.View.extend({
  ...
  <%= namespace %>.views.CardVoteView = Backbone.View.extend({
    ...
    canVote: function() {
      var curBoard = <%= namespace %>.utils.getCurrentBoardModel();
      var result;
      if (curBoard.attributes.voteStatus === 'opened') {
        result = 'opened';
      } else if (curBoard.attributes.voteStatus === 'disabled') {
        result = 'closed';
      } else if (curBoard.attributes.voteStatus === 'enabled' && window.<%= namespace %>.isBoardMember) {
        result = 'opened';
      } else {
        result = 'disabled';
      }
    ...
    voteYes: function() {
      ...
      default:
        $('#card-agree, span.agree').addClass('checked');
        this.voteFlag = 'agree';
        var newVote = new <%= namespace %>.models.Vote({
          cardId: this.options.card.id,
          authorId: <%= namespace %>.utils.getCurrentUser().id
        });
        newVote.save();
